     ...
     xhr.onreadystatechange = function()
	 {
      if(xhr.readyState==4){
                   var obj = eval('(' + xhr.responseText + ')');
					 ...
					 console.log("ddd",str_obj);
					 
					 //填写所有的text类型
					$("input[id='dh_table']").each(function(i){$(this).attr("value",str_obj[$(this).attr('name')]);});
                    //填写所有的textarea类型
          ...
          xhr.open('get','_require_tablename.php?table_name=dh_table',true);
          xhr.send(null);
...
{  
   ...
   var insert_table="dh_table";
   var insert_columns="";
   insert_values="";
   //获取dh_table的内容，并重新生成记录内容
   $("input[id='dh_table']").each(function(index, element) {str_jilu_new[$(this).attr('name')]=$(this).attr('value');insert_columns+=$(this).attr('name')+",";
...
// alert(insert_columns);
 ...
 // alert(str_jilu_new('记录人')); 
  ...
  var insert_values2="\""+$("input[id='dh_table']:eq(3)").val()+"\",";
  ...
  {
   //  	alert(insert_values);
	$.post("insert_data.php",{sql_tb:insert_table,sql_col:insert_columns,sql_val:insert_values},function callback(data,status){alert(status);});//插入dh_table条目
...
<body style="width:1000px; height:600px" onload="getValue()" >


  <div>
<fieldset >
<legend>基本信息</legend>
<label><strong>记录人&nbsp&nbsp </strong> </label><input type="text" id="dh_table" name="记录人" /> 
<label><strong>记录时间&nbsp&nbsp </strong> </label><input type="text"  value=<?php  echo date('c'); ?> id="dh_table" name="记录时间" />
<label><strong>使用人&nbsp&nbsp </strong> </label><input type="text" id="dh_table" name="使用人" /> 
</fieldset>
</div>
...
<div>
    ...
    <legend>详细信息</legend>
        ...
        <tr>
        <td width="30%">
        <label><strong>资产编码&nbsp&nbsp </strong> </label><input type="text" id="dh_table" name="资产编码" /> 
        </td>
        <td width="30%">
        <label><strong>资产姓名&nbsp&nbsp </strong> </label><input type="text" id="dh_table" name="资产姓名" />
        </td>
        <td width="30%">
        <label><strong>设备类型&nbsp&nbsp </strong> </label><select style="width:140px" id="select_type" name="设备类型">
        <option>服务器</option>
        <option>网络设备</option>
        <option>终端设备</option>
        <option>移动设备</option>
        ...
        <option>办公用品</option>
        <option>工具耗材</option>
        <option>环境监控设备</option>
         <option>视频监控设备</option>
         <option>机房配套设备</option>
         <option>动力监控设备</option>
        <option>其他</option>
        </select>
        </td>
        </tr>
        <tr>
        <td><label><strong>规格型号&nbsp&nbsp </strong> </label><input type="text" id="dh_table" name="规格型号" /></td>
        <td><label><strong>设备编码&nbsp&nbsp </strong> </label><input type="text" id="dh_table" name="设备编码" /></td>
        <td><label><strong>设备序列号 </strong> </label><input type="text" id="dh_table" name="设备序列号" /></td>
        </tr>
        <tr>
        <td>
        <label><strong>所在楼房&nbsp&nbsp </strong> </label><select style="width:140px" id="select_type" name="所在楼房">
        <option>管局</option>
        <option>联通二枢纽</option>
        ...
        <td><label><strong>所在房间&nbsp&nbsp </strong> </label><input type="text" id="dh_table" name="所在房间" /></td>
          <td><label><strong>所在位置&nbsp&nbsp </strong> </label><input type="text" id="dh_table" name="所在位置" /></td>
        </tr>
           <tr>
        <td>
        <label><strong>管理部门&nbsp&nbsp </strong> </label><select style="width:140px" id="select_type" name="管理部门">
        <option>技术保障处</option>  
        <option>安全分中心</option>
        <option>国家中心</option>
        <option>其他</option>
          ...
          <td>
          <label><strong>所属主系统</strong> </label><select style="width:140px" id="select_type" name="所属主系统">
        <option>工程系统</option>
        <option>维护系统</option>
        <option>动环系统</option>
        <option>办公系统</option>
        <option>其他</option>
             ...
             <td><label><strong>所属子系统</strong> </label><input type="text" id="dh_table" name="所属子系统" /></td>
        </tr>
          <tr>
        <td><label><strong>原&nbsp&nbsp&nbsp值&nbsp&nbsp </strong> </label><input type="text" id="dh_table" name="原值" /></td>
          <td>
        ...
        <option>已报废</option>
             ...
             <td><label><strong>合同号&nbsp&nbsp&nbsp&nbsp</strong> </label><input type="text" id="dh_table" name="合同号" /></td>
        </tr>
        ...
        <br />
        <label><strong>备注&nbsp&nbsp&nbsp&nbsp </strong> </label>
        <div>
